...
export default {
  ...
  data() {
    return {
      arrImg: ["/hoadat/src/assets/3.JPG", "/hoadat/src/assets/2.JPG"],
      month_names: [
        "Tháng 1",
        "Tháng 2",
        "Tháng 3",
  ...
  methods: {
    ...
    startFlowerFall() {
      ...
      setInterval(() => {
        this.showIndex++;
        if (this.showIndex >= this.arrImg.length) {
          this.showIndex = 0;
        }
    ...
    daysNamNhuan(year) {
      return this.namNhuan(year) ? 29 : 28;
    },
    daysgenerater() {
      let days = [];
      for (let k = 0; k < this.days_of_month().length; k++) {
        ...
        for (let i = 1; i <= this.days_of_month()[k]; i++) {
          if (days[k].length < this.hafta(new Date().getFullYear(), k)) {
            i -= i;
            days[k].push("");
            continue;
          }
    ...
    days_of_month() {
      return [
        31,
        this.daysNamNhuan(new Date().getFullYear()),
        31,
        30,
        31,
        30,
    ...
    isToDay(kun, index) {
      ...
      return dayintable.getDate() == 1 || dayintable.getDate() == 2
        ? true
        : false;
    },
    hafta(sol, ma) {
...
<template>
    ...
    <div class="flex justify-center md:justify-end">
      <div class="text-center">
        ...
        <p>
          ...
          Cảm ơn các mọi người rất nhiều vì sự hiện diện cùng những lời chúc tốt
                ...
                <div v-for="kun of daysgenerater()[index]" :key="kun">
                  <div
                    :class="{
                      ...
                      ' hover:bg-green-100': isToDay(kun, index) == false,
                    }"
                  >
  ...
  <div
    ...
    :key="flower.id"
    class="flower"
    :style="{ left: flower.x + 'px', animationDuration: flower.duration + 's' }"
  >
    🌸
  </div>
</template>
